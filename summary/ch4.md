...
- 우리는 기존 소프트웨어 시스템을 조사하면서, 정의된 boundaries 내에서 서로 다른 모델이 어떻게 적용되는지 이해하려고 노력하는 과정에서, 종종 우리는 모델이 혼합되어 있고, 경계가 일관되지 않는 시스템의 일부분(종종 대규모 시스템)을 발견한다.
...
- 단순히 경계가 없는 시스템에서 모델의 context boundaries를 설명하려는 시도는 수렁에 빠지기 쉽다.
...
- (시스템을 만드는 사람들이 그 당시 이러한 원인에 대해 항상 의식하지는 않을 수 있을지라도) 잘 정의된 맥락 경계는 지적 선택과 사회적 힘에 의해 나타난다.
- 이러한 요소가 없어지거나 사라지면, 여러 개념적 시스템이 섞여서 정의와 규칙이 모호하거나 모순적으로 만든다.
- 기능이 추가됨에 따라, 시스템은 상황에 따른 논리에 의해 작동된다.
- 의존성은 소프트웨어 전반에 걸쳐 있다.
- 원인과 결과를 추적하는 것이 점점 더 어려워지고 있다.
- 결국 소프트웨어는 큰 진흙덩어리로 변모하게 된다.
...
- 큰 진흙덩어리는 실제로 어떤 상황에서는 꽤 유용하지만(푸트와 요더의 원래 논문에서 설명된 대로), 유용한 모델에 필요한 세심함과 정밀함을 거의 완전히 방해한다.
...
- 전체적인 혼돈된 영역의 주위에 boundary를 그리고, 그것을 큰 진흙덩어리로 지정해라.
- 이러한 맥락 내에서 세련된 모델링을 시도하지 마라.
- 이러한 시스템이 다른 맥락으로 확장되는 경향에 주의해라.
